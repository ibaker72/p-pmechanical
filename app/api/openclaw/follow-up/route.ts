...
import { followUpPrompt } from '@/lib/openclaw/prompts';
...
export async function POST(req: Request) {
  ...
  const { system, user } = followUpPrompt(input.data);
  const result = await runPrompt(system, user, { temperature: 0.6, maxTokens: 200 });
